...
async def analyze_youtube(req: AnalyzeRequest):
    ...
    if cached:
        print(f"🎯 Cache HIT: Video {video_id} found in CSV. Skipping audio download and STT.")
        # 영상 메타데이터 취득 (실패 시 캐시 정보 기반 폴백)
        try:
    ...
    # 3. 오디오 전사 (Gemini 3.5 Transcribe)
    try:
        print(f"🎙️ Transcribing audio with Gemini 3.5 Transcribe...")
